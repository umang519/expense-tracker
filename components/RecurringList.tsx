...
export default function RecurringList({ currency = "INR" }: Props) {
  ...
  const FormPanel = (
    <form
      ...
      className="bg-white dark:bg-gray-900 rounded-2xl border border-violet-100 dark:border-violet-800 p-4 space-y-3"
    >
      ...
      <select
        ...
        className="w-full border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-violet-400 bg-white dark:bg-gray-900"
      >
      ...
      <input
        ...
        className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-violet-400"
      />
      ...
      {/* Start date */}
      <div>
        <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">{startDateLabel}</label>
        <input
          type="date"
      ...
      <div>
        <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">
          End date <span className="text-gray-300 dark:text-gray-600">(optional — leave blank to repeat indefinitely)</span>
        </label>
        <input
          type="date"
          value={form.endDate}
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button
          ...
          className="flex-1 py-2.5 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 rounded-xl text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
        >
          Cancel
        </button>
      </div>
    ...
    <div className="space-y-3">
      {!showForm && !editId && (
        <button
          ...
          className="w-full py-3 rounded-2xl border-2 border-dashed border-violet-200 dark:border-violet-800 text-violet-500 dark:text-violet-400 text-sm font-medium hover:border-violet-400 dark:hover:border-violet-700 hover:bg-violet-50 dark:hover:bg-violet-500/10 transition-colors"
        >
      ...
      {isError && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-red-100 dark:border-red-900/50 p-6 text-center">
          <p className="text-sm text-red-500 dark:text-red-400">Could not load recurring expenses.</p>
        </div>
      )}

      {!isLoading && !isError && entries.length === 0 && !showForm && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-8 text-center">
          <p className="text-gray-400 dark:text-gray-500 text-sm">No recurring expenses yet.</p>
          <p className="text-gray-300 dark:text-gray-600 text-xs mt-1">Add rent, SIPs, or subscriptions above.</p>
        </div>
      )}
      ...
      {entries.map((entry) => (
        <div key={entry._id}>
          {editId === entry._id ? FormPanel : (
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 px-4 py-3 flex items-center gap-3">
              <span
              ...
              <div className={`flex-1 min-w-0 ${!entry.isActive ? "opacity-50" : ""}`}>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm font-medium text-gray-800 dark:text-gray-100">{entry.categoryId.name}</span>
                  {!entry.isActive && (
                    <span className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded-full">
                      PAUSED
                    </span>
                  )}
                </div>
              ...
              <div className="relative flex-shrink-0">
                ...
                {menuId === entry._id && (
                  <>
                    ...
                    <div className="absolute right-0 top-8 bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-lg z-10 min-w-[160px] py-1">
                      <button
      ...
      {entries.length > 0 && (
        <div className="bg-violet-50 dark:bg-violet-500/10 rounded-2xl border border-violet-100 dark:border-violet-800 px-4 py-3">
